const games = [  /* Lista de jogos Poderia ser resposta de API, dados de banco */
  {
    "id": 1,
  ...
  return (
    <>
      <div style={{padding: '60px'}}>
          <h1 style={{marginLeft: '40%'}}>Meus Jogos</h1>
          <div style={{display: 'flex' , flexWrap: 'wrap' , gap: '4em'}}>
          {/* Renderizando a Lista de dados com o map()*/}
          {games.map((games) => { /* metodo map retorna um array para dentro do JSX */
            return(
              <div key={games.id} style={{marginTop: '50px'}}>
                <h2>{games.title}</h2>
                <img src={games.coverImage} alt="" style={{height: '300px', width: '200px' , objectFit: 'cover' }}/>
                <p style={{width: '250px'}}>{games.description}<span>{games.releaseYear}</span></p>
              </div>
            )
          })}
          </div>
